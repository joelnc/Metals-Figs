...
## Four panel
graphics.off()
png("ReedyCharDisCopperFig.png", width=6.5, height=5,
     units="in", res=400)
...
FIMD <- which(allCu$Type.x=="Grab - Filtered" &
              ...
              allCu$Coldate>as.POSIXct("2015-01-01 0:00") &
              allCu$Site.x=="MY13")

xLims <- as.POSIXct(c("2015-04-01 00:00", "2017-04-01 23:00"))
xLims <- as.numeric(xLims)
yLims <- c(-10,10)
plot(allCu$Coldate[FIMD],
...
points(allCu$Coldate[FIMD],
       allCu$Result.x[FIMD]-allCu$aStan[FIMD],
       cex=.8, pch=16, col="black")
axThing <- as.POSIXct(c("2016-01-01", "2017-01-01"))
axis(1, at=as.numeric(axThing), labels=FALSE )
text(x=axThing, y=par("usr")[3]*1.25,
     labels=c("Jan. 2016","Jan. 2017"),
     srt=0, xpd=TRUE, cex=1.1)
axis(side=1, labels=FALSE, tcl=-.2,
     at=seq(from=as.POSIXct("2015-01-01"),to=as.POSIXct("2017-04-01"),by="quarter"))
axis(2,las=1,font=2)
box()
text(x=par("usr")[1] + ((par("usr")[2]-par("usr")[1])*0.85),
